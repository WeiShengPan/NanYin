...
<head>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      $.ajax({
        type:'POST',
        url:'pagination.do?method=showGalleryList',
        data:{
          type:${type},
          curPage:curPage,
          pageSize:pageSize,
        ...
        success:function(data) {
          ...
          for(var i=1;!(i >=result.length);i++){
            var tr='<tr><td><a href="index.do?method=galleryLink&id='+result[i].id+'">'+result[i].title+'</a></td><td>'+result[i].author+'</td>' +
                    '<td>'+result[i].date+'</td><td>'+result[i].hits+'</td></tr>';
            $("#galleryList table tbody").append(tr);
          }
          show();
        }
      });
    ...
    function getDate(){
      $.ajax({
        type:'POST',
        url:'pagination.do?method=galleryTurnPage',
        data:{
          type:${type},
          curPage:curPage,
          pageSize:pageSize,

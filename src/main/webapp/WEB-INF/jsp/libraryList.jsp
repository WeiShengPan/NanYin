...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="/css/libraryList.css"/>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      $.ajax({
        type:'POST',
        url:'pagination.do?method=showLibraryList',
        data:{
          type:${type},
          curPage:curPage,
          pageSize:pageSize,
        ...
        success:function(data) {
          ...
          for(var i=1;!(i >=result.length);i++){
            var tr='<tr><td><a href="index.do?method=libraryLink&id='+result[i].id+'">'+result[i].title+'</a></td><td>'+result[i].author+'</td>' +
                    '<td>'+result[i].date+'</td><td>'+result[i].hits+'</td></tr>';
            $("#libraryList table tbody").append(tr);
          }
          show();
        }
      });
    ...
    function getDate(){
      $.ajax({
        type:'POST',
        url:'pagination.do?method=libraryTurnPage',
        data:{
          type:${type},
          curPage:curPage,
          pageSize:pageSize,

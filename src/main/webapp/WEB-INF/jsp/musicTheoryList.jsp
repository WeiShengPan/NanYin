...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="/css/musicTheoryList.css"/>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      $.ajax({
        type:'POST',
        url:'pagination.do?method=showMusicTheoryList',
        data:{
          type:${type},
          curPage:curPage,
          pageSize:pageSize,
        ...
        success:function(data) {
          ...
          for(var i=1;!(i >=result.length);i++){
            var tr='<tr><td><a href="index.do?method=musicTheoryLink&id='+result[i].id+'">'+result[i].title+'</a></td><td>'+result[i].author+'</td>' +
                    '<td>'+result[i].date+'</td><td>'+result[i].hits+'</td></tr>';
            $("#musicTheoryList table tbody").append(tr);
          }
          show();
        }
      });
    ...
    function getDate(){
      $.ajax({
        type:'POST',
        url:'pagination.do?method=musicTheoryTurnPage',
        data:{
          type:${type},
          curPage:curPage,
          pageSize:pageSize,
    ...
    function firstPage(){
      if(curPage!=1){
        curPage=1;
        $("#musicTheoryList table tbody").empty();
        getDate();
      }
    }
    function lastPage(){
